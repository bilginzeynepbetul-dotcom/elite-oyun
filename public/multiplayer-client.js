...
(function () {
  ...
  // İstemci asla maç sonucu/puan yazamaz — applyLeagueResult sarmalayıcı
  (function guardClientResults() {
    const orig = window.applyLeagueResult;
    if (typeof orig === "function") {
      window.applyLeagueResult = function (a, b, gA, gB) {
        if (
          window.__emServerAuthoritative &&
          typeof window.__emIsServerOnline === "function" &&
          window.__emIsServerOnline()
        ) {
          console.warn("[em] applyLeagueResult blocked (server authoritative)");
          return false;
        }
        return orig.apply(this, arguments);
      };
    }
    // apiFetch: POST body içinde skor alanı varsa reddet (istemci tarafı)
    const _af = window.apiFetch;
    if (typeof _af === "function") {
        ...
        try {
          if (
            ...
            if (body && typeof body === "object") {
              ...
              if (bad && String(path || "").indexOf("/team") < 0) {
                console.error(
                  "[em] CLIENT_MATCH_RESULT engellendi:",
                  path,
                  body,
                );
                throw new Error(
                  "Maç sonucu yalnızca sunucu tarafından yazılır",
                );
              }
            }
          }
          ...
          if (
            eGuard &&
            eGuard.message &&
            eGuard.message.indexOf("Maç sonucu") >= 0
          )
            throw eGuard;
        }
