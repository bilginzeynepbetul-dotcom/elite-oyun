...
/**
 * Verilen ülke/lig için aktif sezon yoksa oluşturur (idempotent).
 * Şu ana kadar yalnızca Türkiye 1. lig için başlangıç sezonu seed
 * ediliyordu (001_init_schema.sql); yeni desteklenen ülkelerde
 * (countries.js) kullanıcı kaydolduğunda botClubs.ensureLeagueFilled
 * çağrısı bu fonksiyonla o ülkenin sezonunu otomatik açar — aksi
 * halde standings/fikstür hiç üretilmiyordu.
 */
async function ensureSeason(country = "Türkiye", division = 1, yearLabel = "2025/26") {
  const existing = await getCurrentSeason(country, division);
  ...
  const { rows } = await query(
    `INSERT INTO seasons (country, division, year_label, is_current)
     ...
     RETURNING id, country, division, year_label, is_current`,
    [country, division, yearLabel],
  );
  return rows[0] || null;
}
...
async function getStandings(seasonId) {
  const { rows } = await query(
    `SELECT ls.club_id AS "clubId",
            c.name,
            c.user_id AS "userId",
            COALESCE(c.is_bot, FALSE) AS "isBot",
...
async function generateFixturesForSeason(seasonId, opts = {}) {
  return withTransaction(async (client) => {
    ...
    // --- Gerçek takvim slotları (ülkeye göre, ardışık gün yok) ---
    ...
    if (slots == null) {
      try {
        ...
        if (typeof cal.slotsForCountry === "function") {
          slots = cal.slotsForCountry(seasonRows[0].country);
        }
      } catch (_) {}
    }
    if (slots == null) {
      try {
        const seasonConfig = require("../seasonConfig");
        if (seasonConfig.getLeagueMatchSlots) {
...
async function listFixtures(seasonId, opts = {}) {
  ...
  let sql = `
    SELECT f.id, f.season_id AS "seasonId",
           ...
           COALESCE(hc.is_bot, FALSE) AS "homeIsBot", COALESCE(ac.is_bot, FALSE) AS "awayIsBot",
