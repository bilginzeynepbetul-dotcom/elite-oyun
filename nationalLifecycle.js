...
async function startNationalFixtureMatch(opts) {
  ...
  const homeTeam = {
    ...
    attackDir: (team && team.attackDir) || "orta",
  ...
  try {
    ...
    applyNormalizedTacticsToTeam(homeTeam);
    applyNormalizedTacticsToTeam(awayTeam);
  } catch (_) {}
...
async function onNationalMatchEnd(state, fixture, matchInstance) {
  ...
  try {
    // Maç bitince o ülkenin A + U21 dostluk zincirini yenile
    // (eski kod sabit COUNTRY="Türkiye" kullanıyordu — diğer ülkeler kırılıyordu)
    let country = COUNTRY;
    try {
      const nt = await nationalRepo.getTeamById(fixture.nationalTeamId);
      if (nt && nt.country) country = nt.country;
    } catch (_) {}
