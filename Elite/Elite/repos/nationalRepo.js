...
/**
 * Belirtilen zaman penceresinde (varsayılan: şu andan +36 saat) İLK 11'de
 * çağrılı olan oyuncu id'leri — kulüp maçlarında bu oyuncular saha dışı
 * bırakılır (gerçek "milli araya denk geldi" gerilimi).
 */
async function getPlayersOnDutyIds(windowHours = 36) {
  const { rows } = await query(
    `SELECT DISTINCT ns.player_id
     FROM national_squad ns
     JOIN national_fixtures nf ON nf.national_team_id = ns.national_team_id
     WHERE ns.is_starter = TRUE
       AND nf.status IN ('scheduled', 'live')
       AND nf.kickoff_at BETWEEN NOW() - INTERVAL '6 hours' AND NOW() + ($1 || ' hours')::interval`,
    [windowHours],
  );
