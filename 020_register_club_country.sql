-- ============================================================
-- Elite Manager Online — Migration 020
-- register_new_club fonksiyonuna p_country parametresi eklenir.
-- ------------------------------------------------------------
-- Artık yeni kulüp (fallback: uygun bot yoksa) kullanıcının kayıt
-- sırasında seçtiği ülkeyle oluşturuluyor ve o ülkenin 1. lig
-- sezonuna bağlanıyor (önceden sabit 'Türkiye' idi).
-- ============================================================
...
BEGIN
  ...
  v_country := COALESCE(NULLIF(TRIM(p_country), ''), 'Türkiye');

  INSERT INTO clubs (user_id, name, country, division)
  VALUES (p_user_id, v_name, v_country, 1)
  ...
  FROM seasons s
  WHERE s.country = v_country AND s.division = 1 AND s.is_current
  ORDER BY s.id DESC
  LIMIT 1;
